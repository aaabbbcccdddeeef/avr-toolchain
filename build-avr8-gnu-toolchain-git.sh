...
#                     --docdir=${PREFIX}/doc/gcc/${gcc_version_base}"
    ...
    # Build.
    make ${gcc_extra_config} all html >> make.out 2>&1 || task_error "make failed. See make.out." "make.out"
    task_success
...
function prep_headers () {
    ...
    if [ "${PRIVATE_BUILD}" == "Y" ]; then
      ...
      do
        cp -f $i ${srcdir}/avr-libc/include/avr/ || task_error "Copying non-public headers to avr-libc"
      done
    fi
...
function replace_avr_headers()
{
    ...
    do
      ...
      # if ! test -a ${avr_libc_srcdir}/include/avr/$header_name; then
        cp -f $i ${avr_libc_srcdir}/include/avr/ || task_error "Replacing avr-libc headers with avr-headers"
      # else
...
do
    case ${part} in
        ...
        "avr-gdb" )
            ...
            gdb_build "avr" "--with-python=no"

            # Set with_python for Linux 64 bit and Windows.
            case ${host_platform} in
                i68[3456]-*mingw32 )
                    with_python="$MINGW_PYTHON_CONFIG"
                    ;;
                x86_64*-linux* )
                    with_python="yes"
            esac
            ...
            [[ -n $with_python ]] && \
                gdb_build "avr" "--with-python=$with_python --program-prefix=avr-  --program-suffix=-py"
            ;;
        * )
            echo "Error: Unknown package ${part}."
            exit 1
